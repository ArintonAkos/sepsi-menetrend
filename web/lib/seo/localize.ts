 ...
 *  page is built as Hungarian and `scripts/localize-html.mjs` rewrites the
 *  crawlable attributes on `out/ro/**` and `out/en/**` afterwards. This is the
 *  tested spec of that transform - the `.mjs` re-implements the same two
 *  regexes because a plain ESM script cannot import a `.ts` module.
 *
 *  Both functions are pure and idempotent: `f(f(x)) === f(x)`. */
...
export function toEnglish(html: string): string {
  return html
    ...
    // The `og:locale` meta, when the page did not already declare `en_US`.
